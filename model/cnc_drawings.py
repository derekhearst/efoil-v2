...
def build(p):
    ...
    add("16_module_lid_guide", "MDF 12 mm", 12.0, 1, d, lid_l, lid_w,
        ...
        f"THIS DRILLS THE HOLES. The CNC cuts lid 10's PROFILE only, "
        f"registered by touching off on two black plugs - then this drops "
        f"onto that finished profile, which it IS, and self-locates. Profile "
        f"is registered to the plugs, this is registered to the profile, so "
        f"the holes are registered to the plugs without anyone aiming at "
        f"something buried. MARK WHICH WAY ROUND IT GOES: a rounded "
        f"rectangle drops on equally well at 180 deg and the bolt pattern "
        f"will not tell you. "
    ...
    add("17_hatch_lid_master", "MDF 12 mm", 12.0, 1, d, lw, lh,
        ...
        f"THIS DRILLS THE HOLES. The CNC cuts 02's PROFILE only, registered "
        f"by touching off on two black plugs; this then drops onto that "
        f"finished profile, which it IS, and self-locates - so the holes end "
        f"up over the plugs without anyone aiming at something buried. MARK "
        f"WHICH WAY ROUND IT GOES: a rounded rectangle drops on equally well "
        f"at 180 deg. "
        f"IT IS ALSO THE FALLBACK if the shop will not run cured glass or "
        f"the layup misses the CNC month - then it does both jobs: on the "
        f"BARE CORE before layup, drill the {len(hb)} x "
