...
def build(p):
    ...
    add("12_mast_plate", "6061-T651 aluminium", p["G10_T"], 1, d, gl, gw,
        ...
        f"O{p['BUNG_FREE_D']:.0f} plug pressed UP this bore from below with the "
        f"leads already through it - it is punched "
        f"{p['BUNG_FREE_D'] - p['BUNG_BORE']:.1f} mm OVERSIZE and the bore is "
        f"what compresses it, so the bore's FINISH is now structural: REAM OR "
        f"BORE IT, do not leave it as a drilled hole. That wall is the seal. "
