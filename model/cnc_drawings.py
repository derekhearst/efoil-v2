...
def build(p):
    ...
    add("12_mast_plate", "6061-T651 aluminium", p["G10_T"], 1, d, gl, gw,
        ...
        f"CONDUIT: O{p['PLATE_CONDUIT_D']:.0f} THROUGH on centre, then "
        f"COUNTERBORED O{p['BUNG_BORE']:.1f} x {p['BUNG_L'] - p['BUNG_SQUEEZE']:.2f} "
        f"DEEP FROM THE WETTED FACE. That counterbore takes the wire bung and "
        f"the step at its top is what the bung butts against - the depth is a "
        f"real dimension, not a >= , because the bung has to stand "
        f"{p['BUNG_SQUEEZE']:.1f} mm proud of the wetted face when it is in. "
        f"THAT PROUD BIT IS THE WHOLE SEAL: the GONG MAST'S OWN PLATE bolts "
        f"up through this one and squeezes it as it comes face to face with "
        f"the board. Cut the counterbore too deep and there is nothing left "
        f"to squeeze. REAM OR BORE both diameters - do not leave them drilled. "
