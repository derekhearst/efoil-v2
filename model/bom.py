...
def build():
    ...
    # an epoxy liner step that HAS NO GEOMETRY - the dense block is bored
    # clean through - and even had it existed it would have been a 365 mm2
    # ring carrying 1.72 MPa into H80 that gives up at 1.4.
    #
    # His fix, and it is better than the flanged version I drew first: bore
    # the DENSE BLOCK wider than the plate rather than flanging the blocker
    # wider than the block. NOTHING IS MACHINED INTO THE PLATE - it keeps one
    # straight bore, and the only change of diameter anywhere in this conduit
    # is at the foam-to-alu joint. The blocker lands on the plate's own flat
    # top face: 389 mm2 of 6061 at 1.63 MPa against a 276 MPa yield, where the
    # foam flange it replaces was at 2.0x on H80.
    # It also killed the O-ring. The blocker beds on 4200 into the foam
    # counterbore - an adhesive use, not a wire seal - so the annulus round it
    ...
    add("7  Module", "3M 4200 FC 3 oz tube, skim over the bung", 1, "tube",
        ...
        "on it and the bung's own holes seal the leads. This line is that "
        "bedding plus a skim over the bung's face",
        vendor="Amazon")
